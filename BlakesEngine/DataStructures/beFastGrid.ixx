...
export template<typename T, int LENGTH>
class beFastGrid
{
	static constexpr int STRIDE_LENGTH = NextPowerOfTwo(LENGTH);
	static constexpr int CAPACITY = STRIDE_LENGTH * STRIDE_LENGTH;
	public:
		using value_type = T;
		...
		// Iterates as key/value pairs, but ICE's VC 16.9.3
		struct GridIterator
		{
			beFastGrid* grid;
